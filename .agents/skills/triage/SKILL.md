...
These are canonical role names. Use the repository's configured status names
when available; otherwise use these names directly as local Markdown status
values.
...
4. **Grill (if needed).** If the request needs fleshing out, invoke the `grilling` and `domain-modeling` skills, and grill it into shape a round of questions at a time, sharpening domain terms and updating `CONTEXT.md`/ADRs inline as decisions land.
