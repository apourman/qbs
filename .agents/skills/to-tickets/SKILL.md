...
- Each slice cuts a narrow but COMPLETE path through the relevant layers and
  includes proportionate verification: vertical, NOT a horizontal slice of one layer
...
Publish the approved tickets using the repository's configured tracker. When
no tracker configuration is present, use the local Markdown convention below;
